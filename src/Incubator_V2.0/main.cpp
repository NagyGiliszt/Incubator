...
*/
  ...
  #include <RBD_Servo.h>
  RBD::Servo servo1(5, 1000, 2000); //pin 5, 1ms - 2ms pulse (SG90)
#endif
#ifdef USE_EGGMOTOR
  #ifndef USE_HUMIDIFIER
    #include <RBD_Servo.h>
  #endif
  RBD::Servo servo2(6, 1000, 2000);
#endif
...
void setup() {
  ...
  pinMode(LEDR, OUTPUT);
  pinMode(LEDhideg, OUTPUT);
  pinMode(LEDmeleg, OUTPUT);


  digitalWrite(vent, LOW);
  digitalWrite(heat, LOW);
  digitalWrite(LEDR, LOW);
  digitalWrite(LEDhideg, LOW);
  digitalWrite(LEDmeleg, LOW);
  ...
  lcd.setCursor(4,0);
  lcd.print(F("M")); lcd.write(1); lcd.print(F("h keltet"));lcd.write(2);
  lcd.setCursor(3,1);
  lcd.print(F("SW.Ver.: 2.0.2"));
  lcd.setCursor(3,2);
  lcd.print(F("HW.Ver.: 2.0.0"));
  lcd.setCursor(3,3);
  lcd.print(F("Sensor: "));lcd.print(sensor.name);
  delay(3000);
...
void run_incubation(){
  ...
  if ( T1 < (-30.0) ){
    ...
    lcd.setCursor(0, 2);
    //   Vent xx 
    if (StoreData.pwm_vent >= 25) {
      lcd.print(F("Vent "));lcd.print(int(StoreData.pwm_vent/2.5)); lcd.print(F("%"));
    } else {
      lcd.print(F("Vent "));lcd.print(F("0")); lcd.print(F("%"));
    }
    lcd.print(F(" F"));lcd.write(3);lcd.print(F("t ")); lcd.print(int(pwmOut/1.9)); lcd.print(F("%"));
    lcd.setCursor(0, 3); 
    //fűtőtest hőm
    ...
    if ((airTemp >= (StoreData.kozep - StoreData.hyst)) && (airTemp <= (StoreData.kozep + StoreData.hyst))) {
      ...
      digitalWrite(LEDhideg, HIGH);
      digitalWrite(LEDmeleg, LOW);
    } else if (airTemp >= (StoreData.kozep + StoreData.hyst)) {
      digitalWrite(LEDhideg, LOW);
      digitalWrite(LEDmeleg, HIGH);
    }
  } 
  wait_ms(500L);
